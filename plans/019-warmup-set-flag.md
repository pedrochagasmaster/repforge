...
| Disagreement | Choice |
|--------------|--------|
| Storage field (`kind: "working"\|"warmup"` vs `warmup: true` boolean) | **PR #16 `warmup: true`** — absent means working set; no migration writes on old rows |
| Shared predicate name | **PR #16 `isWork(r)=>!r.warmup`** — grep-able at every consumer |
| Toggle UI | **PR #16 set-number button** — tap chip cycles Working ↔ Warmup ("W"); zero new columns |
| CSV column | **PR #16 `is_warmup`** — aligns with existing `is_hard_set` naming |
| Draft persistence key | **PR #16 `__warm`** — matches existing `__done` / `__touched` pattern |
| Consumer exclusion list | **PR #16 exhaustive seven-site list** — plus PR #15's explicit `summaries()` top-load / fatigue-watch note |
...
- Consumers that treat every `load>0` row as a working set — the exclusion
  ...
  - `renderFatigue` / `renderAttention` — recommendation inputs (via `sessionsFor`).
...
Add a recommendation check: log 3 warmup sets + 2 working sets at top range →
...
- Plan 020 (PR ledger) must ignore warmup sets for PR detection — hard
