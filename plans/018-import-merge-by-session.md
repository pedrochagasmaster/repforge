# Plan 018: Import merge by session id
...
Merged from PR #15 plan 020 and PR #16 plan 020.

| Disagreement | Choice |
|--------------|--------|
| Import UX (two Settings buttons vs in-app chooser dialog) | **PR #16 chooser dialog** — shows session counts and three explicit actions; reuses glossary popover pattern |
| Collision policy when session id exists on both sides | **PR #16 device-wins** — skip incoming rows for that session id; safer than PR #15's "newer `created` timestamp wins" which can silently drop local rows |
| Merge scope | **Both agree** — log-only merge; keep current program and settings untouched |
...
- Branch: `advisor/018-import-merge` (or the operator's requested branch).
...
- Merge adds only-new sessions; collision keeps device copy; merge does not wipe program edits on device.
- Invalid backup still rejected; empty incoming log merge → unchanged.
...
- Per-row fields added by later plans (`warmup`, `performedName` from plans
  019/021) must survive merge unchanged — merge appends rows as-is.
