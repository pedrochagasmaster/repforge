# Plan 021: Exercise substitution flow (approved alternates per slot)
...
- Import merge (plan 018) must preserve `performedName` on log rows
- Warmup flag (plan 019): substituted rows carry `warmup` independently
- Mesocycle blocks (plan 023 spike → 024 build) should key off `exerciseId`, not display name
