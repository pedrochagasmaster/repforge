# Plan 021: Mesocycle blocks — design spike (no production code)
...
- `plans/022-mesocycle-blocks.md` (create — the draft build plan)
- `plans/README.md` (status row + register plan 022 as DRAFT)
...
- Branch: `advisor/021-mesocycle-spike` (or the operator's requested branch).
...
import/merge (Plan 020) compatibility, CSV export, forward compatibility of
...
Write `plans/022-mesocycle-blocks.md` using the structure of an existing
...
**Verify**: `plans/022-mesocycle-blocks.md` exists and its steps are
...
- [ ] `plans/022-mesocycle-blocks.md` exists, marked DRAFT, structurally
      matching the repo's plan template
- [ ] `git status` shows only the three in-scope paths created/modified
- [ ] `plans/README.md` updated: 021 status + 022 registered as DRAFT
...
- The three data-model candidates all fail a hard requirement (e.g. Plan 020
...
- The follow-up build (022) should be executed only after a human reviews the
  design doc — the DRAFT status encodes that.
- Whichever data model wins, Plan 020's merge (union by session id) and
