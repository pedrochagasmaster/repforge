...
- `importJson` / `exportJson` (`app.js:636-647`) — the full-backup path stays
  as-is; Plan 018 changes it separately.
...
- You find yourself modifying `importJson`/`exportJson` — that's Plan 018's
  territory.

## Maintenance notes

- Plan 018 (import merge) redesigns the *full-backup* import dialog; this
...
- Future "beginner program variant" (plan 022) would ship as one of these
