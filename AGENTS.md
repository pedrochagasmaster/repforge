...
- Run the app in development by serving the intended worktree explicitly over HTTP: `python3 -m http.server <port> --directory <absolute-worktree>`. Confirm each selected suite's URL requirements, set `REPFORGE_URL` where supported, and inspect the suite for fixed origins. Compare the served `index.html` and every changed application asset with the files in that worktree before treating results as base or candidate evidence. `waitForAppBoot` checks the Taurifer shell and readiness, not the Git revision; another Taurifer worktree can pass. `affected` starts a preview only for an unreachable default origin and does not replace an existing server.
