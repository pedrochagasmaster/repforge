...
- Prefer production-backed end-to-end/browser journeys as the sole proof of product behavior. Strengthen an existing journey before creating another test file.
- Never add a unit test after writing the implementation merely to cover the code. When isolation is genuinely necessary, first enumerate the production failure modes the browser cannot reasonably exercise, write the failing proof, and only then implement the code.
- An isolated product test must cover a failure E2E cannot reasonably or deterministically own: canonical serialization, migration compatibility, combinatorial/generative invariants, algorithmic boundaries, protocol fault injection, or strict persistence/race behavior.
- Test observable outcomes, not implementation shape. Do not freeze source strings, regexes over production files, helper names, call placement, CSS declaration order, fixture internals, or a completed plan/PR's historical implementation unless that representation is itself a compatibility contract.
- A regression test belongs at the highest realistic boundary that reproduces the bug. Keep a narrower isolated regression only when it adds a distinct failure mode.
- Every browser contract run emits `.ci-results/<lane>/<suite>/evidence.json` with the exact command, source identity, outcome, duration, and output-log digest. Domain-specific screenshots, traces, state snapshots, seeds, or network evidence should be written under `REPFORGE_ARTIFACT_DIR` when they materially prove the contract.
- Test count and coverage percentage are not retention criteria. Delete redundant tests when another retained contract catches the same plausible production failure.
- Syntax, generated-file consistency, dependency pins, payload budgets, and CI self-tests are repository checks, not evidence that a product feature works.
...
- **Agent verification loop:** During correction, run the exact owning contract, or `node tools/run-tests.mjs edit` when ownership is unclear. After a coherent commit, run `node tools/run-tests.mjs packet --base <packet-start-sha>`. At handoff, establish a clean candidate SHA and dispatch candidate CI for that exact SHA once; do **not** run the exhaustive local `candidate` gate first unless diagnosing a candidate-only failure. Source changes create a new candidate. `edit` is feedback, `packet` is the normal broad local proof, and `branch`/local `candidate` are diagnostic tools. Focus an exact failure with `--suite-id` and use runner `--evidence /tmp/proof.json` on a clean commit when provenance is required. Do not retry a failed Actions run to seek green; diagnostic replay never changes red to green. Inspect retained logs/traces. Do not repeatedly poll Actions while useful independent work remains. See `docs/ci.md` and `test/AGENTS.md`.
