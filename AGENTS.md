...
`docs/business-product-thesis.md` is the current strategic source of truth for market, business model, Free/Pro boundaries, validation sequencing, creator distribution, data principles, and the eventual native direction. `docs/adr/0010-product-business-thesis-and-validation-sequencing.md` records the governing decisions. Precedence: this file governs how the repository works today; the thesis and ADR govern why we are building and what strategic direction is authorized. The current repository architecture remains the Phase 1 static PWA until the evidence gates described there justify native commercialization — do not implement Phase 2 platform work (native shells, package managers at the root, backend services) on the strength of the thesis alone, and treat the thesis's quantitative figures as hypotheses to test, not requirements.
