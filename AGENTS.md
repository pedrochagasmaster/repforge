...
One-off sessions have a pure domain foundation that the production page does not load yet. `session-intent.js` (`RepForgeSessionIntent`) owns session kinds, the validated session context, the row eligibility stamp, and the scoped history selectors (`all_training`, `program_completion`, `program_progression`, `active_block`) plus the program-day queue projection; `session-planner.js` (`RepForgeSessionPlanner`) owns deterministic classic, muscle-focus, manual, and planned-day adaptation plans and the explicit plural-to-catalogue equipment mapping. Neither creates a second workout engine: an accepted plan becomes the single DraftV2 aggregate through `WorkoutDraft.create`, which carries an optional `sessionContext` and stamps it onto saved rows. Until the post-overhaul entry integration loads `session-intent.js`, `workout-draft.js` rejects any draft carrying a context, so nothing can execute or save a one-off as planned work. Rows without the new fields keep legacy eligibility. See `docs/superpowers/specs/2026-08-25-one-off-session-design.md`.
...
- **Agent verification loop:** During correction, run the exact owning suite, or `node tools/run-tests.mjs edit` when ownership is unclear. After a coherent commit, run `node tools/run-tests.mjs packet --base <packet-start-sha>`. At handoff, establish a clean candidate SHA and dispatch candidate CI for that exact SHA once; source changes create a new candidate. `edit` is feedback, not completion evidence; `branch` (the old `affected` semantics) is diagnostic, and `candidate` is the exhaustive completion boundary. Focus an exact failure with `--suite-id` and use runner `--evidence /tmp/proof.json` on a clean commit when provenance is required. Do not retry a failed Actions run to seek green; diagnostic replay never changes red to green. Inspect retained logs/traces. Do not wait in `gh run watch` while useful independent local work remains. See `docs/ci.md` and `test/AGENTS.md`.
