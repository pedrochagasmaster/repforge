...
| B10 | S3 §5; S5 SHOULD 2; S6 §2.6 | Failed activation unconditionally restores stale setup-draft bytes and can overwrite another tab's newer draft. | blocker | confirmed | — | Transaction trace; two-tab failure test pending |
| B11 | S3 §5; S1 unproven 3 | Setup draft is removed before asynchronous durability completes, creating a crash-loss window. | blocker | confirmed | — | Finalizer ordering trace; restart/failure test pending |
...
| C09 | S6 consider | Setup-draft save failures are console-only, leaving resumability failure invisible. | medium | confirmed | — | `persistSetupDraft` trace; failure UI test pending |
