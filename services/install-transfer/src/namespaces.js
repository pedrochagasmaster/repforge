...
export function euNamespace(namespace, { allowLocalFallback = false } = {}) {
  ...
  try {
    ...
    if (allowLocalFallback && error instanceof Error && error.message === "Jurisdiction restrictions are not implemented in workerd.") {
      return namespace;
    }
    throw error;
  }
...
export function euStub(namespace, name, options) {
  return euNamespace(namespace, options).getByName(name);
}
