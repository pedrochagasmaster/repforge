...
function openTourOverlay(){
  ...
  tour.setAttribute("aria-modal","true");tour.classList.remove("hidden");
  tourInertPrev=[];
  [...document.body.children].forEach(el=>{if(el===tour)return;tourInertPrev.push([el,el.inert]);el.inert=true});
  $("#tourSkip")?.focus()}
function closeTourOverlay(){
  const tour=$("#tour");if(tour){tour.classList.add("hidden");tour.setAttribute("aria-modal","true")}
  (tourInertPrev||[]).forEach(([el,prev])=>{el.inert=prev});tourInertPrev=null}
