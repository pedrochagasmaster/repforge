...
function renderProgram(){
  if(document.activeElement!==$("#programJson"))$("#programJson").value=JSON.stringify(state.program.map(({id,...x})=>x),null,2);
  const m=new Map();
  for(const e of state.program){for(const x of muscles(e.primary))addVol(m,x,e.sets,0);for(const x of muscles(e.secondary))addVol(m,x,0,e.sets*.5)}
  const arr=[...m.entries()].map(([name,v])=>({name,eff:v.d+v.p,d:v.d,p:v.p})).sort((a,b)=>b.eff-a.eff);
...
function saveProgram(){try{state.program=JSON.parse($("#programJson").value).map((x,i)=>({id:uid(),day:x.day||"Day 1",order:+x.order||i+1,name:x.name||"Exercise",sets:+x.sets||2,min:+x.min||4,max:+x.max||8,primary:x.primary||"",secondary:x.secondary||""}));save();day=days()[0]||"Day 1";render();toast("Program saved.")}catch{toast("That JSON didn't parse. Check the brackets and commas.")}}
...
async function importJson(e){const f=e.target.files?.[0];if(!f)return;try{const s=JSON.parse(await f.text());if(!s.program||!Array.isArray(s.log))throw Error();state=s;save();render();toast("Backup imported.")}catch{toast("That file isn't a valid RepForge backup.")}e.target.value=""}
