...
- Prefer a production-backed browser journey for product behavior. Do not add post-hoc unit tests after implementation; isolated proofs must be designed from enumerated failure modes that E2E cannot reasonably cover.
- Do not assert source shape or preserve a historical implementation because a plan once tested it. Keep tests only for current, distinct failure detection.
- Successful browser contracts must leave the runner evidence artifact plus any contract-specific screenshot/state/network/seed artifact needed to reproduce the claim.
