...
- Browser dependencies are a one-time checkout setup. The test runner starts an isolated current-worktree preview on a fresh loopback port whenever browser suites are selected and `REPFORGE_URL` is unset. If a local `REPFORGE_URL` is supplied, the runner verifies worktree identity before running anything. Browser suites consume that shared origin; do not add private fixed-port servers or non-8000 fallback origins.
