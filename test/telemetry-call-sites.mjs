...
const historyUi = readFileSync(resolve(root, "history-ui.js"), "utf8");
...
const callSites = `${app}\n${historyUi}`;
const captured = [...callSites.matchAll(callPattern)].map(match => match[1]);
...
for (const retired of [
  ...
  assert.doesNotMatch(callSites, new RegExp(`captureEvent\\(\\"${retired}\\"`));
}
