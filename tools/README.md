...
`build-clone-fixture.mjs` through the real runtime normalization path — with
the same frozen-input and hostile-key proofs.
...
application normalization path: it compiles a real `growth_3_v1` program via
`program-compiler.js`, runs the result through `app.js`'s `normalizeLoaded`
in a minimal browser shell, asserts a semantic round-trip
(`normalizeLoaded(fixture.durableState)` deep-equals the fixture, storage
metadata excluded), then fills the canonical digest through
`canonical-hash-core.mjs`. The ADR's displayed envelope is derived from the
fixture, so ADR, fixture, and digest cannot drift apart.
