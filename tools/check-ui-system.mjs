...
export function inspectRoleCoverage({ key, components, exceptions, progressCandidateSelectors, allowProgressDebt = false }) {
  ...
  for (const selector of progressCandidateSelectors) for (const node of document.querySelectorAll(selector)) if (visible(node)) candidate.add(node);
  ...
  const problems = [], observed = [], matched = new Set();
  ...
  for (const node of candidate) {
    ...
    if (progressCandidateSelectors.some((selector) => node.matches(selector)) && !exception) {
  ...
  return { key, problems, observed, matched: [...matched] };
}

export async function auditCatalog({ allowProgressDebt = false, flow = null, onProgress = () => {} } = {}) {
  ...
  const captures = manifest.screens.filter((screen) => !flow || screen.flow === flow)
  ...
  try {
    for (let index = 0; index < captures.length; index++) {
      ...
      try {
        ...
        problems.push(`${key}: scenario failed: ${error.message.split("\n")[0]}`);
  ...
  if (!flow) for (const item of inventory.components) {
    if (!matched.has(item.id) && !item.sourceOnly) problems.push(`inventory selector never rendered: ${item.selector}`);
  }
  return { problems, screens: captures.length, matched: [...matched] };
}
...
if (process.argv[1] && import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  ...
  if (!process.argv.includes("--metadata")) {
    ...
    const result = await auditCatalog({ allowProgressDebt: process.argv.includes("--allow-progress-debt"), flow,
