...
for (const w of [320, 390, 768, 1280]) {
  ...
  const r = await p.evaluate(() => {
    const stage = document.querySelector('.lp-stage--pair');
    const s = stage.getBoundingClientRect();
    const box = sel => {
      const e = stage.querySelector(sel + ':not([style*="display: none"])');
    ...
    const back = box('.lp-stage__back'), front = box('.lp-stage__front');
    const clip = o => ({
