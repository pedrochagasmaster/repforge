...
- **Session progress stays visible.** A sticky rail reports completed sets and the current exercise position.
...
- **Overview is a temporary layer.** It lists the whole session in a sheet and returns the user directly to the selected exercise.
...
- Escape closes Overview and returns focus to its trigger.
...
The static shell remains dependency-free. Cache `repforge-v16-focus-deck` refreshes the redesigned HTML, CSS, JavaScript, manifest, icon, and local fonts while preserving network-first shell updates and cached offline reloads.
