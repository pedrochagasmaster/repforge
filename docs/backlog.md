...
|---|---|---|
| Deliver the owner-approved UI overhaul | Execute Plans 049–058 in DAG order with owner gates honored, then the Direction D redesign below, then Plan 059 (owner decision, 2026-09-25): reconciled contracts, verified defect fixes, DraftV2/transition/transfer foundations, landing/entry, Focus-only workout, truthful Progress, converged management surfaces, system migration, and same-SHA launch validation. No governing current-tense document contradicts G-01–G-88. | [Plan 049](../plans/049-ui-overhaul-canonical-reconciliation.md), [sequence](ui-overhaul-implementation-sequence.md), [dispositions](ui-overhaul-disposition-register.md) |
| Direction D redesign | Direction D ("Folha e polegar"; workfront [PR #272](https://github.com/pedrochagasmaster/repforge/pull/272): spec, review page, [ADR 0016](adr/0016-direction-d-design-reference.md), [Plan 063](../plans/063-direction-d-redesign.md)) is the owner-selected reference for the main screens: Today, Focus and rest, Why this weight, the session summary, Progress and the exercise chart, History and Program. It is built on Plan 058's finished semantic roles and adopts their frozen type, radius and elevation scale wherever D's own numbers differ; anything D needs that 058 lacks (the workout shelf, inline rest) goes through 058's contract review as a new content job. The session-outcome fix is already on the same branch and lands with it. Done when D's screens replace the current ones and its acceptance checks (targets, overflow, orange budget, outcome and target parity) pass against the real app. Depends on Plan 058 merging. Plan 059 validates the result, and nothing ships to lifters before it. Replaces nothing; D's coverage gaps (onboarding, Settings, Library and the other surfaces D does not draw) are inventoried as part of this work. | Owner decisions on PR #264, 2026-09-25 |
...
| Truthful session outcomes | The outcome word on a lift must state what the logged sets show. Today a prescribed load increase with the expected rep drop reads "Regressou" beside its own load PR, and a same-load session with one rep fewer reads "Carga alterada" although the load did not change. Replace the `buildSessionDelta` branch order with the owner-approved rule in [`CONTEXT.md`](../CONTEXT.md) "Session outcome" (same load: total reps decide; load up: best-set e1RM decides at ±1 %; load down: Melhorou only on more strength or more volume at similar effort, otherwise not comparable). In the same PR, "Why this weight" names the performed anchor reps instead of capacity reps, and the EN pause label stops saying "Hold". Outcomes are derived at read time, so no history is rewritten. Replaces nothing. Committed on the Direction D workfront (PR #272) and merges with it, after Plan 058, which prints these words on every screen; nothing ships to lifters before then (owner decision, 2026-09-25). | Owner grilling on PR #264's open questions, 2026-09-25 |
...
foundations are credible and the Direction D redesign has passed Plan 059. There is no synchronized cohort, special research
...
Engineering work that follows the Now data-safety fixes (Q612). These are test
and tooling changes, not post-overhaul product work, so under Q604 they may
start as standalone PRs once the crash-journal fix lands:
