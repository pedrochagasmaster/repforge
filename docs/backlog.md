...
| Truthful session outcomes | The outcome word on a lift must state what the logged sets show. Today a prescribed load increase with the expected rep drop reads "Regressou" beside its own load PR, and a same-load session with one rep fewer reads "Carga alterada" although the load did not change. Replace the `buildSessionDelta` branch order with the owner-approved rule in [`CONTEXT.md`](../CONTEXT.md) "Session outcome" (same load: total reps decide; load up: best-set e1RM decides at ±1 %; load down: Melhorou only on more strength or more volume at similar effort, otherwise not comparable). In the same PR, "Why this weight" names the performed anchor reps instead of capacity reps, and the EN pause label stops saying "Hold". Outcomes are derived at read time, so no history is rewritten. Depends on nothing; standalone PR; replaces nothing. Lands before the Direction D redesign work, which prints these words on every screen. | Owner grilling on PR #264's open questions, 2026-09-25 |
