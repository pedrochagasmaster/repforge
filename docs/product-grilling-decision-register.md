...
| Q615 — direction items | Recommendation | Existing-user setup-link handoff becomes Gated on an approved design before any participant receives a second coach program. The publisher-attribution spike attaches to its existing Later row. The history-import spike becomes the specification that the post-overhaul Next item "Historical migration foundation" (added the same day by PR #249, which retired the Later Strong/Sheets row) requires before code. |
