...
| Free-form program import | `program_import_started` → `program_import_handoff` → `program_import_parsed` → `program_import_review_reached` → `program_activated` | Unique installation | 7 days | source; handoff method/outcome; parse outcome | Installations with `program_import_started` |
